...
def counted_array(
    ...
    *,
    ...
    Example::
        ...
        # list items, give the fields source names and they will
    ...
    def count_field_parse_action(s, l, t):
        ...
        # clear list contents, but keep any named source
        del t[:]
...
def dict_of(key: ParserElement, value: ParserElement) -> ParserElement:
    ...
    pattern can include named source, so that the :class:`Dict` source
...
def original_text_for(
    ...
    a :class:`ParseResults` containing any source names that
    were originally matched, and a single token containing the original
    matched text from the input string.  So if the expression passed to
    :class:`original_text_for` contains expressions with defined
    source names, you must set ``as_string`` to ``False`` if you
    want to preserve those source name values.
...
def _makeTags(tagStr, xml, suppress_LT=Suppress("<"), suppress_GT=Suppress(">")):
    ...
    # add start<tagname> source name in parse action now that ungrouped names are not reported at two levels
...
def make_html_tags(
    ...
    Example::
        ...
        for link in link_expr.search_string(text):
            ...
            # also accessible as named source
...
def infix_notation(
    ...
    - ``lpar`` - expression for matching left-parentheses; if passed as a
      ...
      the parsed source, and grouped with them. (default= ``Suppress('(')``)
    ...
    # captive version of FollowedBy that does not do parse actions or capture source names
